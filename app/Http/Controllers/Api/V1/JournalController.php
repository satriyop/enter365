...
class JournalController extends Controller
{
    ...
    public function index(JournalFilter $filter): AnonymousResourceCollection
    {
        ...
        $journals = Journal::query()
            ->with(['defaultAccount', 'suspenseAccount', 'outstandingReceiptsAccount', 'outstandingPaymentsAccount', 'profitAccount', 'lossAccount'])
            ->filter($filter)
            ->orderBy('name')
    ...
    public function store(StoreJournalRequest $request): JsonResponse
    {
        ...
        return (new JournalResource($journal->load(['defaultAccount', 'suspenseAccount', 'outstandingReceiptsAccount', 'outstandingPaymentsAccount', 'profitAccount', 'lossAccount'])))
            ->response()
            ->setStatusCode(201);
    }
    ...
    public function show(Journal $journal, JournalFilter $filter): JournalResource
    {
        ...
        $journal->loadMissing(['defaultAccount', 'suspenseAccount', 'outstandingReceiptsAccount', 'outstandingPaymentsAccount', 'profitAccount', 'lossAccount']);
