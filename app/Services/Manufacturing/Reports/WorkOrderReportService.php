...
class WorkOrderReportService
{
    ...
    public function getWorkOrderCostSummary(
        ...
        $workOrderData = $workOrders->map(function (WorkOrder $wo) {
            return [
                ...
                'product_name' => $wo->product->name ?? null,
                'type' => $wo->type,
                'status' => $wo->status,
        ...
        return [
            ...
            'totals' => [
                ...
                'variance_percent' => $totalEstimated > 0
                    ...
                    : 0,
                'completed_count' => $workOrders->where('status', DocumentStatus::Completed)->count(),
                'in_progress_count' => $workOrders->where('status', DocumentStatus::InProgress)->count(),
            ],
        ];
    }
    ...
    public function getCostVarianceReport(
        ...
        $query = WorkOrder::query()
            ->whereIn('status', [DocumentStatus::Completed, DocumentStatus::InProgress])
