...
class SubcontractorReportService
{
    ...
    public function getSubcontractorSummary(
        ...
        $subcontractorData = $subcontractors->map(function (Contact $sub) {
            ...
            $completedCount = $workOrders->where('status', DocumentStatus::Completed)->count();
            $totalCount = $workOrders->count();

            // Calculate average completion time for completed WOs
            $completedWOs = $workOrders->where('status', DocumentStatus::Completed);
            $avgCompletionDays = 0;
            ...
            return [
                ...
                'work_orders' => [
                    ...
                    'in_progress' => $workOrders->where('status', DocumentStatus::InProgress)->count(),
                    'draft' => $workOrders->where('status', DocumentStatus::Draft)->count(),
                ],
                'financials' => [
    ...
    public function getSubcontractorDetail(
        ...
        return [
            ...
            'summary' => [
                ...
                'completed_work_orders' => $workOrders->where('status', DocumentStatus::Completed)->count(),
                'total_agreed' => $totalAgreed,
                'total_actual' => $totalActual,
                'total_invoiced' => $totalInvoiced,
                'total_paid' => $totalPaid,
    ...
    public function getRetentionSummary(): array
    {
        ...
        $retentionData = $workOrders->map(fn ($wo) => [
            ...
            'scheduled_end' => $wo->scheduled_end_date?->format('Y-m-d'),
            'actual_end' => $wo->actual_end_date?->format('Y-m-d'),
            'is_releasable' => $wo->status === DocumentStatus::Completed,
        ]);
        ...
        $totalRetention = $workOrders->sum('retention_amount');
        $releasableAmount = $workOrders->where('status', DocumentStatus::Completed)
